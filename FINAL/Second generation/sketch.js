...
function generateSkyCircles() {
  // Use a different seed value to ensure it doesn't affect the Perlin noise of the river
  noiseSeed(99);
  let clouds = 5000; // generate 5000 clouds
  for (let i = 0; i < clouds; i++) {
    // Find a random position for each cloud based on Perlin noise
    let x = noise(i * 0.1) * 1500;
    let y = noise(i * 0.1 + 100) * 350; 
    let cloudSize = noise(i * 0.1 + 200) * 5; // Size based on noise value
    // Draw the cloud
    fill(255, 255, 255, 150); // White with semi-transparency
    ellipse(x, y, cloudSize, cloudSize / 2); // Clouds are elliptical
  }
  // Reset the seed so as not to affect other noise-based code
