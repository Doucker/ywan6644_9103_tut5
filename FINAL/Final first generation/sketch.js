...
for (let i = 0; i < 100; i++) {
  smokeParticles.push({
    ...
    velocity: createVector(0, random(-1, -3)),
    size: random(1, 5)
  });
}
...
function drawSmoke() {
  // Traverse all smoke particles
  for (let i = 0; i < smokeParticles.length; i++) {
    let p = smokeParticles[i];
    // Adjust particle position using noise function
    let n = noise(p.position.x * 0.01, p.position.y * 0.01, frameCount * 0.01);
    p.velocity.x = map(n, 0, 1, -1, 1);
    p.position.add(p.velocity);
    
    // Draw the particle. Adjust color and size as needed
    noStroke();
    // Set your desired color for the smoke using RGBA values (e.g., a shade of gray with variable opacity)
    fill(200, 200, 200, n * 255); // Adjust the RGBA values as needed
    
    // Increase the base size of the particles and amplify it with noise
    let baseSize = 10; // Base size of the particles
    let sizeVariation = n * baseSize; // Variable size based on noise
    ellipse(p.position.x, p.position.y, baseSize + sizeVariation, baseSize + sizeVariation);

    // If the particle moves off the top of the canvas, reposition it at the bottom
    if (p.position.y < 0) {
      p.position.y = height;
      p.position.x = random(width);
      // Randomize the size a bit when repositioning
      p.size = random(baseSize, baseSize * 2); // Change the range as needed
    }
  }
}


// Function to generate circular clouds in the sky
function generateSkyCircles() {
  // Use a different seed value to ensure it does not affect the Perlin noise of the river
  noiseSeed(99);
  let clouds = 5000; // Assume we want to generate 5000 clouds
  for (let i = 0; i < clouds; i++) {
    // Find a random position for each cloud based on Perlin noise
    let x = noise(i * 0.1) * 1500;
    let y = noise(i * 0.1 + 100) * 350; // Generate on the upper half of the screen
    let cloudSize = noise(i * 0.1 + 200) * 5; // Size based on noise value
    // Draw the cloud
    fill(255, 255, 255, 150); // White with some transparency
    ellipse(x, y, cloudSize, cloudSize / 2); // Clouds are elliptical
  }
  // Reset seed to avoid affecting other noise-based code
...
function river() {
  for (let i = 0; i < num; i++) {
      ...
      if (vecLocation[i].x < 0 || vecLocation[i].x > width) {
          vecVelocity[i].x *= -1; // Reverse the velocity in the X direction // Add new color transition logic here
          if (vecLocation[i].x < 0) {
              ...
              if (
                  ...
                  // Lower range orange-red
                  R[i] = 225;
                  G[i] = 82;
                  B[i] = 47;
              } else {
                  // Lighter purple below
                  R[i] = 99;
                  G[i] = 44;
                  B[i] = 159;
              }
